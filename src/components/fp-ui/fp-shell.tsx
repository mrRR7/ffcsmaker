...
const navItems = [
  { href: "/new", label: "Home" },
  { href: "/new/planner", label: "Plan" },
  { href: "/new/results", label: "Results" },
  { href: "/new/compare", label: "Compare" },
  { href: "/new/saved", label: "Saved" },
  { href: "/new/settings", label: "Settings" }
];
...
export function FPShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-fp-bg-page text-fp-text-body">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={active ? "text-fp-accent" : "text-fp-text-dim hover:text-fp-text-body"}
              >
                {item.label}
              </Link>
            );
                  ...
                  {campusOptions.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isActive ? <span className="text-fp-accent">✓</span> : null}
                      </button>
                    );
                  })}
                </div>
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 pb-16 sm:px-6 lg:px-8">
        {mainContent}
      </main>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          {campusOptions.map((option) => (
            ...
            >
              ...
              <FPLabel tone={option.active ? "accent" : "dim"} className="mt-3.5 inline-block">
                {option.active ? "Ready →" : "Not yet"}
              </FPLabel>
            </button>
          ))}
        </div>
