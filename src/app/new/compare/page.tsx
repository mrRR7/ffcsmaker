  ...
  return (
    <div className="-mx-4 -my-8 sm:-mx-6 lg:-mx-8">
      <div className="flex items-center justify-end border-b border-fp-border-default px-7 py-3">
        <span className="font-fp-mono text-[var(--text-micro)] text-fp-text-dim">
      ...
      <section className="px-7 pb-4 pt-7">
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <h1 className="font-fp-display text-[var(--text-display)] font-bold tracking-[-0.01em] text-fp-text-strong">{headline}</h1>
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <select
              ...
              className="fp-label h-full rounded-[var(--radius-md)] border border-fp-border-strong bg-transparent px-3 py-[7px] text-[var(--text-micro)] text-fp-text-body hover:border-fp-accent"
            >
        ...
        {diffs.length > 0 ? (
          <div className="mt-5 flex flex-wrap items-center gap-5 rounded-[6px] border border-fp-border-default bg-fp-bg-surface px-4 py-3">
            ...
            {diffs.map((diff) => (
              <span key={diff.label} className="text-[var(--text-small)] text-fp-text-body">
                {diff.label} &middot;{" "}
      ...
      {allSchedules.length === 0 ? (
        <div className="px-7 pb-10">
          <FPCard className="flex min-h-72 flex-col items-center justify-center gap-3 text-center">
            <p className="font-fp-display text-[var(--text-h)] font-bold text-fp-text-strong">No weeks to compare yet</p>
            <p className="max-w-sm text-[var(--text-small)] text-fp-text-dim">
        ...
        <div className="px-7 pb-10">
          <FPCard className="flex min-h-56 flex-col items-center justify-center gap-2 text-center">
            ...
            <p className="max-w-sm text-[var(--text-small)] text-fp-text-dim">Choose up to three weeks from the selector above.</p>
          </FPCard>
        </div>
      ) : (
          ...
          {selected.length < 3 && selected.length < allSchedules.length
            ? Array.from({ length: 3 - selected.length }).map((_, i) => (
                <button
                  ...
                  className="flex min-h-24 items-center justify-center rounded-[var(--radius-lg)] border border-dashed border-fp-border-strong text-[var(--text-small)] text-fp-text-dim transition-colors hover:border-fp-accent hover:text-fp-text-body"
                >
